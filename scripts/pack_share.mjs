...
保存后双击 **\`命令/配置与定时/生成会话清单.cmd\`**。

### 3. 双击更新
- 微博：双击 **\`命令/备份与更新/更新备份.cmd\`**
- B站 ：双击 **\`命令/备份与更新/更新B站备份.cmd\`**（首次要在弹出的浏览器窗口里扫码登录一次）
...
| \`使用限制.md\` | **用之前请读**：可以做 / 请不要做，以及法律责任 |
| \`LICENSE\` | MIT 许可证全文 |
| \`scripts/init.mjs\` | 一键铺工作区（装好之后跑这一个） |
| \`assets/\` \`命令/\` \`scripts/\` | 查看页、双击启动器（按用途分 4 类）、全部逻辑 |
...
代码以 **MIT** 开源（见 \`LICENSE\`），你可以自由使用、修改、分发，包括商用，保留版权声明即可。

但请务必读一下 \`使用限制.md\` —— **这个工具是备份你自己的聊天记录用的，
不是用来抓别人私信的。** 分享备份给别人之前也请先打码，因为里面装的是对方的隐私。
`;
