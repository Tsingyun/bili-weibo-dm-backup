...
function readZipInner(buf) {
  ...
  for (let n = 0; n < count; n++) {
    ...
    if (!name.endsWith('/')) {
      ...
      let data;
      // ⚠ maxOutputLength 必须给：中央目录里声明的 rawSize 是**不可信**的，
      //   恶意包可以声明 1MB 实际解出几十 GB，在下面 total 检查之前就把内存打爆。
      //   解出超限时 inflateRawSync 会抛 ERR_BUFFER_TOO_LARGE，正好挡死这条路。
      if (method === 8) data = zlib.inflateRawSync(comp, { maxOutputLength: MAX_ENTRY });
      else if (method === 0) {
        if (comp.length > MAX_ENTRY) throw new Error('单文件过大，拒绝解压：' + name);
