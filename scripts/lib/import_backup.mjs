...
import { jsAssign } from './jsassign.mjs';
...
function ensureWrappers(dirAbs, key, entries) {
  ...
  const made = [];
  // ⚠ 包内的 *.js 一律视为**不可信输入**：不管包里有没有自带的 .js，
  //   包装文件都用 .json 数据重新生成并覆盖 —— 绝不原样复用包里的 .js，
  //   否则等于在查看页同源执行别人塞进来的任意代码。
  //   生成的文件用 `<script src>` 加载，正文里只要出现 `</script>`（消息文本里
  //   完全可能有），HTML 解析器就会当场把 script 块切断 —— 所以 `<` → \u003c
  //   的转义统一在 lib/jsassign.mjs 里做，这里不再手写 replace。
  for (const [base, gname] of pairs) {
    const jsonPath = path.join(dirAbs, base + '.json');
    const fallback = base === 'messages' ? '[]' : '{}';
    let raw = fallback;
    if (fs.existsSync(jsonPath)) {
      raw = fs.readFileSync(jsonPath, 'utf8').trim() || fallback;
    }
    fs.writeFileSync(path.join(dirAbs, base + '.js'), jsAssign(gname, raw), 'utf8');
    made.push(base + '.js');
  }
  return { made, globals };
}
