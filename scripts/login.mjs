...
export async function launchBrowser(session) {
  ...
  const args = [
    ...
    // ⚠ 只放行本机来源的 CDP 连接。之前是 `*`（任何来源都接受）——
    //   调试端口虽只监听 127.0.0.1，外网进不来，但**本机任意网页**都能连上来
    //   读 Cookie、驱动这个浏览器。换成显式值后，浏览器内嵌的恶意页面
    //   （Origin 对不上）会被 Chrome 直接拒掉；而 cdp_lib 用的是 Node 的
    //   WebSocket，握手**不带 Origin 头**，非浏览器客户端不受这个名单影响
    //   （2026-09 已实测：Node 22 全局 WebSocket 发出的握手里没有 Origin）。
