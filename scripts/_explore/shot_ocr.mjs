/**
 * 真实 Chromium 渲染「搜图片文字」效果：截图 + 量真实渲染色
 */
import fs from 'fs';
...
const meta = await page.evaluate(() => {
  const OCR = window.DM_OCR || {};
  const keys = Object.keys(OCR).filter(k => k !== '_meta');
  return {
    keys: keys.length,
    withText: keys.filter(k => (OCR[k] || {}).t).length,
    stats: document.getElementById('stats').textContent,
  };
});
console.log('索引 ' + meta.keys + ' 条，含文字 ' + meta.withText + ' 张');
console.log('侧栏统计：' + meta.stats.replace(/\s+/g, ' '));

await page.screenshot({ path: OUT + '/ocr-1-stats.png' });
...
if (word) {
  ...
  await page.screenshot({ path: OUT + '/ocr-2-search.png' });
  ...
  const geo = await page.evaluate(() => {
    ...
    const img = document.querySelector('.imgs img.ocrhit');
    ...
    const out = {
      ...
      hitImgs: document.querySelectorAll('.imgs img.ocrhit').length,
      marks: document.querySelectorAll('.ocrtip mark').length,
      tip: g(tip), wrap: g(wrap), img: g(img),
      tipText: tip ? tip.textContent.slice(0, 80) : '',
  ...
  await page.screenshot({ path: OUT + '/ocr-3-zoom.png' });
}
...
await page.screenshot({ path: OUT + '/ocr-4-dark.png' });
