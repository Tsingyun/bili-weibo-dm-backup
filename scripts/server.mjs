 ...
 */
function pipeFile(req, res, full, asAttachment, forceType = null) {
  let st;
  try { st = fs.statSync(full); } catch { return fail(res, 404, '文件不存在'); }
  res.on('error', () => { try { res.destroy(); } catch {} });
  res.writeHead(200, {
    'Content-Type': forceType || MIME[path.extname(full).toLowerCase()] || 'application/octet-stream',
...
// 「不可信」数据目录：里面的文件可能是导入包带来的，不看内容、只看类型 ——
// 白名单类型（查看页真正要用的脚本 / 数据 / 图片）照常内联返回；
// 其余（.html / .svg / .txt / .md / .zip / 未知类型…）一律按
// application/octet-stream + attachment 输出，只能存盘、不能在 WebUI 同源上执行。
// 查看页自己的页面（查看备份.html / webui/ / assets/）不在此列。
const UNTRUSTED_PREFIXES = ['data/', 'bili/', 'imported/', 'exports/', 'imports/'];
const INLINE_OK = new Set(['.js', '.mjs', '.json', '.jsonl',
...
function serveStatic(req, res, u) {
  ...
  const rp = rel(full).replace(/\\/g, '/');
  if (SENSITIVE_RE.test(rp)) return sendText(res, '403 Forbidden', 403);
  // 不可信目录 + 非白名单类型 → 强制下载，绝不内联（.html/.svg 尤其不行）。
  if (UNTRUSTED_PREFIXES.some((x) => rp.startsWith(x)) && !INLINE_OK.has(path.extname(full).toLowerCase())) {
    return pipeFile(req, res, full, true, 'application/octet-stream');
  }
...
const server = http.createServer(async (req, res) => {
  // Host 校验：本服务只应从 127.0.0.1 / localhost 访问。
  // DNS rebinding 会把攻击者域名解析到 127.0.0.1 —— 浏览器判定「同源」，
  // 读接口（/api/preview 等）又没有 CSRF 头可查，此时唯一可靠的信号就是 Host：
  // 正常访问的 Host 一定是本机地址 + 实际监听端口，攻击者域名对不上，一条断言挡死。
  const host = String(req.headers.host || '');
  if (host !== '127.0.0.1:' + ACTUAL_PORT && host !== 'localhost:' + ACTUAL_PORT) {
    return sendText(res, '403 Forbidden', 403);
  }
  let u;
