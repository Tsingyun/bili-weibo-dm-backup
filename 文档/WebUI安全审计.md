...
### A8 调试端口 `--remote-allow-origins` —— 中风险 · **已收紧（2026-09-22）**

- **定位**：`scripts/login.mjs` 启动 Chrome 时的参数；`bili.ps1` / `update.ps1` / `_explore` 两个启动器同款。
- **风险**：CDP 端口只监听 127.0.0.1（外网连不上），但 `*` 意味着**同机的任意网页**都能连上来操作那个浏览器
  （读 Cookie、以你的登录态发请求）。
- **本次修复**：全部换成显式值 `--remote-allow-origins=http://127.0.0.1:<port>,http://localhost:<port>` ——
  浏览器内嵌的恶意页面（Origin 对不上）会被 Chrome 直接拒绝。
  **实测依据**：cdp_lib 走 Node 22 内置 WebSocket，握手**不带 Origin 头**（探针脚本验证过），
  非浏览器客户端不受该名单影响，CDP 链路无需改动、登录流程零风险。
- **规避**：只在需要扫码登录时开那个浏览器窗口，登完就关。
...
1. ~~**A8 CDP 调试端口**（中风险）~~ —— **已于 2026-09-22 收紧**（见 A8），不再保留。
2. **B站缺图 1 张 + OCR 孤儿键**（低风险，数据问题）。
3. **接口速率限制**：未做。判断为可接受 —— 服务只监听 127.0.0.1、单用户本机使用，且写接口要求
   自定义头 `X-DM-WebUI: 1` + 同源 `Origin`，已挡住浏览器侧的 CSRF；2026-09-22 起 GET 还加了
   `Host` 校验（挡 DNS rebinding，见「全项目隐私与合规安全审计.md」M1）。若以后要暴露到局域网，**必须先加限速和鉴权**。
